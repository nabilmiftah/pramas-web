...
const Registration = () => {
  ...
  const handleFileChange = (idAnggota, jenisDokumen, file) => {
    if (file) {
      // Perbarui state daftarAnggota dengan file yang dipilih
                ...
                {daftarAnggota.map((anggota) => (
                  <div key={anggota.id} className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {/* Input KTP */}
                        <label className={`border-2 border-dashed rounded-xl p-4 flex flex-col items-center justify-center gap-2 cursor-pointer transition-colors ${anggota.fileKtp ? 'border-emerald-500 bg-emerald-50' : 'border-gray-300 hover:border-emerald-500 text-gray-500'}`}>
                          <svg className={`w-6 h-6 ${anggota.fileKtp ? 'text-emerald-600' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
                          <span className={`font-semibold text-sm text-center line-clamp-1 ${anggota.fileKtp ? 'text-emerald-700' : 'text-gray-500'}`}>
                            {anggota.fileKtp ? anggota.fileKtp.name : `Unggah KTP ${anggota.nama}`}
                          </span>
                          <input 
                            type="file" 
                            accept=".jpg,.jpeg,.png,.pdf" 
                            className="hidden" 
                            onChange={(e) => handleFileChange(anggota.id, 'fileKtp', e.target.files[0])}
                          />
                        </label>

                        {/* Input Surat Sehat */}
                        <label className={`border-2 border-dashed rounded-xl p-4 flex flex-col items-center justify-center gap-2 cursor-pointer transition-colors ${anggota.fileSehat ? 'border-emerald-500 bg-emerald-50' : 'border-gray-300 hover:border-emerald-500 text-gray-500'}`}>
                          <svg className={`w-6 h-6 ${anggota.fileSehat ? 'text-emerald-600' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                          <span className={`font-semibold text-sm text-center line-clamp-1 ${anggota.fileSehat ? 'text-emerald-700' : 'text-gray-500'}`}>
                            {anggota.fileSehat ? anggota.fileSehat.name : 'Unggah Surat Sehat'}
                          </span>
                          <input 
                            type="file" 
                            accept=".jpg,.jpeg,.png,.pdf" 
                            className="hidden" 
                            onChange={(e) => handleFileChange(anggota.id, 'fileSehat', e.target.files[0])}
                          />
                        </label>
                    </div>
                  </div>
